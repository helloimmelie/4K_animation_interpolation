...
wandb.init(project="stopmotion_interp", entity="helloimmelie")
...
def parse_args():
    ...
    parser.add_argument('--phase', type=str, default='train', choices=['train', 'test', 'test_custom', 'metrics_evaluation',])
    ...
    parser.add_argument('--train_data_path', type=str, default=r'C:\Users\mandy\Dataset\X4K1000FPS\train')
    parser.add_argument('--val_data_path', type=str, default=r'C:\Users\mandy\Dataset\X4K1000FPS\val')
    parser.add_argument('--test_data_path', type=str, default=r'C:\Users\mandy\Dataset\X4K1000FPS\test')
    ...
    parser.add_argument('--custom_path', type=str, default='/HDD/sujincho/AnimationDataset', help='path for custom video containing frames')
...
def train(model_net, criterion, device, save_manager, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        wandb.log({"PSNR":testPSNR})
        wandb.log({"SSIM":testSSIM})
...
def test(test_loader, model_net, criterion, epoch, args, device, multiple, postfix, validation):
    ...
    with torch.no_grad():
        ...
        for testIndex, (frames, t_value, scene_name, frameRange) in enumerate(test_loader):
            ...
            pred_frameT, flow_l = model_net(input_frames, t_value, is_training=False)
            ...
            if args.phase != 'test_custom':
                ...
                scene_save_path = os.path.join(epoch_save_path, scene_name[0])
                pred_frameT = np.squeeze(pred_frameT.detach().cpu().numpy())
                flow_l = flow2img(np.transpose(flow_l[0,:2,:,:].detach().cpu().numpy(),[1,2,0]))
                output_img = np.around(denorm255_np(np.transpose(pred_frameT, [1, 2, 0])))  # [h,w,c] and [-1,1] to [0,255]
                print(os.path.join(scene_save_path, It_Path[0]))
                cv2.imwrite(os.path.join(scene_save_path, It_Path[0]), output_img.astype(np.uint8))
                cv2.imwrite(os.path.join(scene_save_path,'flow'+It_Path[0]),flow_l)
                losses.update(0.0, 1)
                PSNRs.update(0.0, 1)
                SSIMs.update(0.0, 1)
