...
from models.pwc_modules import conv, initialize_msra, upsample2d_flow_as, upsample_flow, FlowEstimatorDense_v2, ContextNetwork_v2_, OccEstimatorDense, OccContextNetwork
from models.pwc_modules import WarpingLayer_no_div, FeatureExtractor
    ...
    class sgu_model(tools.abstract_model):
        def __init__(self):
            ...
            self.upsample_output_conv = nn.Sequential(conv(3, 16, kernel_size=3, stride=1, dilation=1),
                                                      conv(16, 16, stride=2),
                                                      conv(
                                                          16, 32, kernel_size=3, stride=1, dilation=1),
                                                      conv(32, 32, stride=2), )
        ...
        def forward(self, flow_init, feature_1, feature_2, output_level_flow=None):
            ...
            if h != h_f or w != w_f:
                flow_init = upsample2d_flow_as(
                    flow_init, feature_1, mode="bilinear", if_rate=True)
            ...
            if output_level_flow is not None:
                ...
                inter_mask = upsample2d_flow_as(
                    inter_mask, output_level_flow, mode="bilinear")
                flow_init = output_level_flow
            flow_up = tools.torch_warp(
...
class XVFInet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.channel_converter = nn.Sequential(
            nn.Conv3d(self.in_channels, self.nf, [
                      1, 3, 3], [1, 1, 1], [0, 1, 1]),
            nn.ReLU())

        self.rec_ext_ds_module = [self.channel_converter]
        self.rec_ext_ds = nn.Conv3d(
            self.nf, self.nf, [1, 3, 3], [1, 2, 2], [0, 1, 1])
        for _ in range(int(np.log2(self.scale))):
            self.rec_ext_ds_module.append(self.rec_ext_ds)
            self.rec_ext_ds_module.append(nn.ReLU())
        self.rec_ext_ds_module.append(
            nn.Conv3d(self.nf, self.nf, [1, 3, 3], 1, [0, 1, 1]))
        self.rec_ext_ds_module.append(
            RResBlock2D_3D(args, T_reduce_flag=False))
        self.rec_ext_ds_module = nn.Sequential(*self.rec_ext_ds_module)

        self.rec_ctx_ds = nn.Conv3d(
            self.nf, self.nf, [1, 3, 3], [1, 2, 2], [0, 1, 1])
    ...
    def forward(self, x, t_value, is_training=True):
        ...
        if is_training:
            out_l_list = []
            flow_refine_l_list = []
            out_l, flow_l, flow_refine_l = self.vfinet(
                x, feat_x_list[self.args.S_trn], flow_l, t_value, level=self.args.S_trn, is_training=True)
            out_l_list.append(out_l)
            flow_refine_l_list.append(flow_refine_l)
            # self.args.S_trn, self.args.S_trn-1, ..., 1. level 0 is not included
            for level in range(self.args.S_trn-1, 0, -1):
                out_l, flow_l = self.vfinet(
                    x, feat_x_list[level], flow_l, t_value, level=level, is_training=True)
                out_l_list.append(out_l)
            out_l, flow_l, flow_refine_l, occ_0_l0 = self.vfinet(
                x, feat_x_list[0], flow_l, t_value, level=0, is_training=True)
            out_l_list.append(out_l)
            flow_refine_l_list.append(flow_refine_l)
            # out_l_list should be reversed. [out_l0, out_l1, ...]
            return out_l_list[::-1], flow_refine_l_list[::-1], occ_0_l0, torch.mean(x, dim=2)

        else:  # Testing

            flow_refine_l_list = []
            # self.args.S_tst, self.args.S_tst-1, ..., 1. level 0 is not included
            for level in range(self.args.S_tst, 0, -1):
                flow_l = self.vfinet(
                    x, feat_x_list[level], flow_l, t_value, level=level, is_training=False)
                flow_refine_l_list.append(flow_l)

            out_l = self.vfinet(
                x, feat_x_list[0], flow_l, t_value, level=0, is_training=False)

            return out_l, flow_refine_l_list[-1]
...
class VFInet(nn.Module):
    ...
    def forward(self, x, feat_x, flow_l_prev, t_value, level, is_training):
        ...
        if level == 0:
            ...
            x_l = F.interpolate(x_l, scale_factor=(
                1.0 / l, 1.0 / l), mode='bicubic', align_corners=False)
        '''
		Down pixel-shuffle
		'''
        ...
        if flow_l_prev is None:
            flow_l_tmp = self.conv_flow_bottom(
                torch.cat((feat0_l, feat1_l), dim=1))
            flow_l = flow_l_tmp[:, :4, :, :]
        else:
            flow_l_prev = upsample2d_flow_as(
                flow_l_prev, feat_x, mode="bilinear", if_rate=True)
            flow_l = self.self_guided_upsample(
                flow_up_bilinear=flow_l_prev.detach(), feature_1=feat0_l, feature_2=feat1_l)
            warped_feat1_l = self.bwarp(feat1_l, flow_l_prev[:, :2, :, :])
            warped_feat0_l = self.bwarp(feat0_l, flow_l_prev[:, 2:, :, :])
        # warped_feat0_l = channel_change_net(flow_l[: ,:2 , :, :], self.args.nf) #custom depth
        # warped_feat1_l = channel_change_net(flow_l[:, 2:, :, :], self.args.nf) #custom depth
        # warped_feat0_l = flow_l[: ,:2 , :, :] #custom depth
        # warped_feat1_l = flow_l[:, 2:, :, :] #custom depth
            flow_l_tmp = self.conv_flow2(torch.cat([self.conv_flow1(torch.cat([feat0_l, warped_feat1_l], dim=1)), self.conv_flow1(
                torch.cat([feat1_l, warped_feat0_l], dim=1)), flow_l_prev], dim=1))
            flow_l = flow_l_tmp[:, :4, :, :] + flow_l_prev
        ...
        # Complementary Flow R eversal (CFR)
        # Actually, F (t) -> (t+1). Translation only. Not normalized yet
        flow_forward, norm0_l = self.z_fwarp(
            flow_01_l, t_value * flow_01_l, z_01_l)
        # Actually, F (1-t) -> (-t). Translation only. Not normalized yet
        flow_backward, norm1_l = self.z_fwarp(
            flow_10_l, (1-t_value) * flow_10_l, z_10_l)

        # The numerator of Eq.(1) in the paper.
        flow_t0_l = -(1-t_value) * ((t_value)*flow_forward) + \
            (t_value) * ((t_value)*flow_backward)
        # The numerator of Eq.(2) in the paper.
        flow_t1_l = (1-t_value) * ((1-t_value)*flow_forward) - \
            (t_value) * ((1-t_value)*flow_backward)

        norm_l = (1-t_value)*norm0_l + t_value*norm1_l
        mask_ = (norm_l.detach() > 0).type(norm_l.type())
        # Divide the numerator with denominator in Eq.(1)
        flow_t0_l = (1-mask_) * flow_t0_l + mask_ * \
            (flow_t0_l.clone() / (norm_l.clone() + (1-mask_)))
        # Divide the numerator with denominator in Eq.(2)
        flow_t1_l = (1-mask_) * flow_t1_l + mask_ * \
            (flow_t1_l.clone() / (norm_l.clone() + (1-mask_)))
        ...
        # Flow refinement
        flow_refine_l = torch.cat(
            [feat0_l, warped0_l, warped1_l, feat1_l, flow_t0_l, flow_t1_l], dim=1)
        flow_refine_l = self.conv_flow3(
        ...
        # Flow upscale
        flow_t0_l = self.scale * F.interpolate(flow_t0_l, scale_factor=(
            self.scale, self.scale), mode='bilinear', align_corners=False)
        flow_t1_l = self.scale * F.interpolate(flow_t1_l, scale_factor=(
            self.scale, self.scale), mode='bilinear', align_corners=False)
        ...
        refine_out = self.refine_unet(torch.cat([F.pixel_shuffle(torch.cat([feat0_l, feat1_l, warped0_l, warped1_l], dim=1),
                                                                 self.scale), x_l[:, :, 0, :, :], x_l[:, :, 1, :, :], warped_img0_l, warped_img1_l, flow_t0_l, flow_t1_l], dim=1))
        occ_0_l = torch.sigmoid(refine_out[:, 0:1, :, :])
        occ_1_l = 1-occ_0_l

        out_l = (1-t_value)*occ_0_l*warped_img0_l + \
            t_value*occ_1_l*warped_img1_l
        out_l = out_l / ((1-t_value)*occ_0_l + t_value *
                         occ_1_l) + refine_out[:, 1:4, :, :]
    ...
    def z_fwarp(self, img, flo, z):
        ...
        # firstly, get gaussian weights
        w11, w12, w21, w22 = self.get_gaussian_weights(
            x, y, x1, x2, y1, y2, z+1e-5)
    ...
    def sample_one(self, img, shiftx, shifty, weight):
        ...
        flat_basex = torch.arange(0, H, requires_grad=False).view(
            -1, 1)[None, None].to(self.device).long().repeat(N, C, 1, W).view(-1)
        flat_basey = torch.arange(0, W, requires_grad=False).view(
            1, -1)[None, None].to(self.device).long().repeat(N, C, H, 1).view(-1)
        flat_weight = weight.view(-1)
        flat_img = img.contiguous().view(-1)

        # The corresponding positions in I1
        idxn = torch.arange(0, N, requires_grad=False).view(
            N, 1, 1, 1).to(self.device).long().repeat(1, C, H, W).view(-1)
        idxc = torch.arange(0, C, requires_grad=False).view(
            1, C, 1, 1).to(self.device).long().repeat(N, 1, H, W).view(-1)
        ...
        img_warp.put_(ids_mask, torch.masked_select(
            flat_img * flat_weight, mask), accumulate=True)

        one_warp = torch.zeros([N * C * H * W, ]).to(self.device)
        one_warp.put_(ids_mask, torch.masked_select(
            flat_weight, mask), accumulate=True)
...
class RefineUNet(nn.Module):
    def __init__(self, args):
        ...
        self.enc1 = nn.Conv2d((4*self.nf)//self.scale//self.scale +
                              4*args.img_ch + 4, self.nf, [4, 4], 2, [1, 1])
        ...
        # input concatenated with enc2
        self.dec1 = nn.Conv2d(4*self.nf + 2*self.nf, 2 *
                              self.nf, [3, 3], 1, [1, 1])
        # input concatenated with enc1
        self.dec2 = nn.Conv2d(2*self.nf + self.nf, self.nf, [3, 3], 1, [1, 1])
        # input added with warped image
        self.dec3 = nn.Conv2d(self.nf, 1+args.img_ch, [3, 3], 1, [1, 1])
...
class RResBlock2D_3D(nn.Module):
    ...
    def __init__(self, args, T_reduce_flag=False):
        ...
        if T_reduce_flag:
            self.reduceT_conv = nn.Conv3d(
                self.nf, self.nf, [3, 1, 1], 1, [0, 0, 0])
